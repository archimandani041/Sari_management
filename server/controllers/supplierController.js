...
const updateSupplier = async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase.from('suppliers').select('id').eq('id', id).eq('owner_id', req.user.owner_id).single();
...
const deleteSupplier = async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase.from('suppliers').select('id').eq('id', id).eq('owner_id', req.user.owner_id).single();
    if (!existing) return res.status(404).json({ error: 'Supplier not found' });
    const { error } = await supabase.from('suppliers').update({ is_active: false }).eq('id', id).eq('owner_id', req.user.owner_id);
    if (error) throw error;
